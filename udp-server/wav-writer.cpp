...
QWav::QWav(const QString &fname, QFile *parent)
    : QFile(parent),
      m_filename(fname),
      m_stream(this)
{
...
void QWav::append(short data[], int size)
{
...
void QWav::write_hdr()
{
    union {
        struct wav_hdr_t hdr;
        char c[sizeof(struct wav_hdr_t)];
    } u;
    int samples_per_second = WAVFILE_SAMPLES_PER_SECOND;
    int bits_per_sample = 16;
    u.hdr.riff_tag[0] = 'R';
    ...
    u.hdr.riff_len = 0;
    u.hdr.fmt_len = 16;
    u.hdr.audio_format = 1;
    u.hdr.num_channels = 1;
    u.hdr.sample_rate = samples_per_second;
    u.hdr.byte_rate = samples_per_second * (bits_per_sample/8);
    u.hdr.block_align = bits_per_sample / 8;
    u.hdr.bits_per_sample = bits_per_sample;
    u.hdr.data_len = 0;
    this->seek(0);
    m_stream << u.c;
